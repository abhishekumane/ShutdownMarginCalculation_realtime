...
class DataShare:
    ...
    def animate(self,i):
        ...
        # self.ax2.plot(self.list_mem_number, self.list_mem['UAVLEG1']['Val'],label='Loop1 Tavg',linewidth=1)
        ...
        #self.ax2.plot(self.list_mem_number, self.list_mem['UCOLEG3']['Val'],label='Loop3 Tcold',linewidth=1)  # 이거는 ax2에 여러값을 찍고싶었구나
        #self.ax1.plot(self.list_mem_number, self.list_mem['UCOLEG3']['Val'],label='Cold-Leg3 T')
        #self.ax1.legend(loc='upper center',bbox_to_anchor=(0.5,1.09),ncol=5,fontsize=10) # 범례 위치도 바꿀수있지
        #self.ax2.legend(loc='upper center', bbox_to_anchor=(0.5, 1.09), ncol=5, fontsize=10)  # 범례 위치도 바꿀수있지
